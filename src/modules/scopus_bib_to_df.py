...
def scopus_bib_to_df(file_path):
    ...
    This function reads a BibTeX file, extracts its entries, and organizes them into a DataFrame with columns
    based on a predefined mapping. It processes the file by converting all data to uppercase, handling multiple entries
    for fields like 'funding_details', and extracting relevant information such as the affiliation universities.
    It also changes the author format from "Last, F." to "LAST F" and constructs new columns such as 'SR_FULL' and 'SR'
    ...
    try:
        ...
        column_order = [
            ...
            'FX', 'AU_UN', 'AU1_UN', 
            # 'AU_UN_NR',  # Removed as per your request
            'SR_FULL', 'SR',
        ...
        for entry in entries:
            ...
            for key in entry:
                ...
                if key == 'url':
                    ...
                    entry_data[key] = value.upper()

                # Process 'author' field
                if key == 'author':
                    authors_raw = value.replace('\n', ' ')
                    # Split authors by ' and '
                    authors_list = re.split(r'\s+and\s+', authors_raw)
                    processed_authors = []
                    ...
                    entry_data[key] = ';'.join(processed_authors)
